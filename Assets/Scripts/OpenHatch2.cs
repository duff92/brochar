...
public class OpenHatch2 : MonoBehaviour {
	...
	void Update () {
        ...
        if(Input.touchCount > 0)
        {
            ...
            if(Input.GetTouch(0).phase == TouchPhase.Moved && activeHatch)
            {
                ...
                if(hinge.transform.eulerAngles.z < 290 && !hatchOpen)
                {
                    Debug.Log("Hatch open");
                    hatchOpen = true;
                    bottleCtrl.ActivateBottle(hatchOpen);
                }
                else if (hinge.transform.eulerAngles.z <= 359 && hinge.transform.eulerAngles.z > 290)
                {
                    hatchOpen = false;
                    bottleCtrl.ActivateBottle(hatchOpen);
                }
